...
function refreshAlbumInformation(albumID, tabPage)
{
	...
	window.albumClient.onreadystatechange = function() {
		if (window.albumClient.readyState == 4 && window.albumClient.status == 200) {
			...
			a = obj.AlbumRetrieveResult; // obj.ResultSet[0];
			...
			// Format
			inputElements[4].value = a.Discs.Disc[0].MusicMedium.Name;		
	
			// Label
			if(a.Publications.Publication.length > 0)
				inputElements[5].value = a.Publications.Publication[0].Publisher.Name;
			
			// Release Date
			inputElements[6].value = a.ReleaseDate.Name;	

			// Region
			inputElements[7].value = a.GeoEntities.GeoEntityValue[0].GeoEntity.Name;
			
			// Catalogue Number
			if(a.Publications.Publication.length > 0)
				inputElements[8].value = a.Publications.Publication[0].CatalogueNumber;		
			
			// Barcode
			var externalIdentifiers = a.ExternalIdentifiers.ExternalIdentifier;
			var barcode = "";
			...
			// Genres
			var genreValues = a.Genres.GenreValue;
			var genres = "";
...
{
	tracks = window.album[tabPage].Tracks.Track;
...
{
	participations = track.Performances.Performance[0].Participations.Participation;
	$(".trackParticipants").flexAddData(formatTrackParticipations(participations));		
